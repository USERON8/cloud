...
WHERE g.app_name = 'order-service'
  AND NOT EXISTS (
      ...
      FROM xxl_job_info j
      WHERE j.job_group = g.id
        AND j.executor_handler = 'orderArchiveJob'
  );
...
WHERE g.app_name = 'search-service'
  AND NOT EXISTS (
      ...
      FROM xxl_job_info j
      WHERE j.job_group = g.id
        AND j.executor_handler = 'sellRankRefreshJob'
  );
...
SELECT
    ...
    0
FROM xxl_job_group g
WHERE g.app_name = 'product-service'
  AND NOT EXISTS (
      SELECT 1
      FROM xxl_job_info j
      WHERE j.job_group = g.id
        AND j.executor_handler = 'minioTempFileCleanJob'
  );
...
WHERE g.app_name = 'stock-service'
  AND NOT EXISTS (
      ...
      FROM xxl_job_info j
      WHERE j.job_group = g.id
        AND j.executor_handler = 'stockAlertJob'
  );
