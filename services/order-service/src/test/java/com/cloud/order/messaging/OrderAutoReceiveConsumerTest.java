...
class OrderAutoReceiveConsumerTest {
    ...
    @BeforeEach
    void setUp() {
        consumer = new OrderAutoReceiveConsumer(messageIdempotencyService, orderService, orderSubMapper);
    }

    @Test
    void shouldAdvanceWhenShipped() {
        OrderAutoReceiveEvent event = OrderAutoReceiveEvent.builder()
                .subOrderId(11L)
                .subOrderNo("S2026000001")
                .build();
        Message<OrderAutoReceiveEvent> message = MessageBuilder.withPayload(event).build();
        when(messageIdempotencyService.tryAcquire(eq("order:auto-receive"), eq("ORDER_AUTO_RECEIVE:S2026000001")))
                .thenReturn(true);
        ...
        verify(messageIdempotencyService).markSuccess(eq("order:auto-receive"), eq("ORDER_AUTO_RECEIVE:S2026000001"));
    }
    ...
    void shouldSkipWhenStatusNotShipped() {
        OrderAutoReceiveEvent event = OrderAutoReceiveEvent.builder()
                .subOrderId(13L)
                .subOrderNo("S2026000003")
                .build();
        Message<OrderAutoReceiveEvent> message = MessageBuilder.withPayload(event).build();
        when(messageIdempotencyService.tryAcquire(eq("order:auto-receive"), eq("ORDER_AUTO_RECEIVE:S2026000003")))
                .thenReturn(true);
        ...
        verify(messageIdempotencyService).markSuccess(eq("order:auto-receive"), eq("ORDER_AUTO_RECEIVE:S2026000003"));
    }
}
