...
public class OrderArchiveXxlJob {
    ...
    public void archiveOrders() {
        ...
        List<Long> mainIds = jdbcTemplate.queryForList(
                """
                SELECT id
                FROM order_main
                WHERE deleted = 0
        ...
        String idClause = mainIds.stream()
                .map(String::valueOf)
                .collect(Collectors.joining(","));

        int archivedMain = jdbcTemplate.update("""
        ...
        int archivedSub = jdbcTemplate.update("""
                ...
                FROM order_sub
                WHERE main_order_id IN (""" + idClause + ")"
        );

        int archivedItems = jdbcTemplate.update("""
        ...
        int deletedItems = jdbcTemplate.update("DELETE FROM order_item WHERE main_order_id IN (" + idClause + ")");
        int deletedSubs = jdbcTemplate.update("DELETE FROM order_sub WHERE main_order_id IN (" + idClause + ")");
