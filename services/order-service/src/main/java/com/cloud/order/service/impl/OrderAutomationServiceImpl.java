...
public class OrderAutomationServiceImpl implements OrderAutomationService {
  ...
  private static final List<String> AUTO_APPROVE_AFTER_SALE_STATUSES =
      List.of("APPLIED", "AUDITING", "APPROVED");
  ...
  public int autoConfirmShippedOrders() {
    ...
    for (OrderSub subOrder : subOrders) {
      try {
        ...
        log.warn(
            "Auto confirm receipt skipped, subOrderId={}, reason={}",
            subOrder.getId(),
            ex.getMessage());
      }
    }
  ...
  public int autoApproveTimedOutAfterSales() {
    LocalDateTime deadline =
        LocalDateTime.now().minusHours(properties.getAfterSale().getAuditTimeoutHours());
    List<AfterSale> afterSales =
        afterSaleMapper.selectList(
  ...
  private boolean processAfterSale(AfterSale afterSale) {
    try {
      ...
      log.warn(
          "Auto approve after-sale skipped, afterSaleId={}, reason={}",
          afterSale.getId(),
          ex.getMessage());
      return false;
    }
