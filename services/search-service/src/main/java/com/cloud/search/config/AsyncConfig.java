...
public class AsyncConfig extends BaseAsyncConfig {
  ...
  public Executor searchIndexExecutor() {
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor("searchIndexExecutor", 4, 16, 500, 60, "search-index-");
    executor.initialize();
  ...
  public Executor searchESBatchExecutor() {
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor("searchESBatchExecutor", 4, 12, 800, 60, "search-es-batch-");
    executor.initialize();
  ...
  public Executor searchSuggestionExecutor() {
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor("searchSuggestionExecutor", 3, 8, 200, 60, "search-suggestion-");
    executor.initialize();
  ...
  @ConditionalOnProperty(
      name = "search.statistics.enabled",
      havingValue = "true",
      matchIfMissing = true)
  public Executor searchStatisticsExecutor() {
    ...
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor(
            "searchStatisticsExecutor", processors, processors + 1, 100, 60, "search-stats-");
    executor.initialize();
  ...
  public Executor searchHotKeywordExecutor() {
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor("searchHotKeywordExecutor", 2, 4, 100, 60, "search-hot-");
    executor.initialize();
  ...
  public Executor searchCacheExecutor() {
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor("searchCacheExecutor", 2, 6, 150, 60, "search-cache-");
    executor.initialize();
