...
public interface CategoryDocumentRepository
  ...
  @Query("{\"bool\": {\"must\": [{\"term\": {\"parentId\": ?0}}, {\"term\": {\"status\": ?1}}]}}")
  Page<CategoryDocument> findByParentIdAndStatusOrderBySortOrder(
      Long parentId, Integer status, Pageable pageable);

  @Query(
      "{\"bool\": {\"must\": [{\"match\": {\"name\": \"?0\"}}, {\"term\": {\"level\": ?1}}, {\"term\": {\"status\": ?2}}]}}")
  Page<CategoryDocument> advancedSearch(
      String name, Integer level, Integer status, Pageable pageable);
}
