...
public interface ShopDocumentRepository extends ElasticsearchRepository<ShopDocument, String> {
  ...
  Page<ShopDocument> findByRatingBetween(
      BigDecimal minRating, BigDecimal maxRating, Pageable pageable);
  ...
  @Query(
      "{\"bool\": {\"must\": [{\"multi_match\": {\"query\": \"?0\", \"fields\": [\"shopName\", \"description\", \"address\"]}}, {\"range\": {\"rating\": {\"gte\": ?1}}}, {\"term\": {\"status\": ?2}}]}}")
  Page<ShopDocument> advancedSearch(
      String keyword, BigDecimal minRating, Integer status, Pageable pageable);
}
