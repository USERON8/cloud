...
public class PaymentOutboxRelay {
    ...
    private boolean sendRefundCompleted(OutboxEvent event) throws Exception {
        RefundCompletedEvent payload = objectMapper.readValue(event.getPayload(), RefundCompletedEvent.class);
        ...
        Message<RefundCompletedEvent> message = MessageBuilder.withPayload(payload)
                .copyHeaders(headers)
                .build();
