...
public class AsyncConfig extends BaseAsyncConfig {
  ...
  public Executor paymentAsyncExecutor() {
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor("paymentAsyncExecutor", 3, 8, 400, 60, "payment-async-");
    executor.initialize();
  ...
  public Executor paymentNotificationExecutor() {
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor(
            "paymentNotificationExecutor", 2, 5, 300, 60, "payment-notification-");
    executor.initialize();
  ...
  public Executor paymentReconciliationExecutor() {
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor(
            "paymentReconciliationExecutor", 2, 4, 200, 60, "payment-reconciliation-");
    executor.initialize();
  ...
  @ConditionalOnProperty(
      name = "payment.statistics.enabled",
      havingValue = "true",
      matchIfMissing = true)
  public Executor paymentStatisticsExecutor() {
    ...
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor(
            "paymentStatisticsExecutor", processors, processors + 1, 100, 60, "payment-stats-");
    executor.initialize();
  ...
  public Executor paymentLogExecutor() {
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor("paymentLogExecutor", 1, 3, 300, 60, "payment-log-");
    executor.initialize();
  ...
  public Executor paymentRefundExecutor() {
    ThreadPoolTaskExecutor executor =
        createConfiguredExecutor("paymentRefundExecutor", 2, 6, 250, 60, "payment-refund-");
    executor.initialize();
