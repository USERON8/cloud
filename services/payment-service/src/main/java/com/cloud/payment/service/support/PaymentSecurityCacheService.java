    ...
    try {
      Boolean ok =
          stringRedisTemplate
              ...
              .setIfAbsent(
                  key,
                  safeValue(idempotencyKey),
                  Duration.ofSeconds(safeSeconds(idempotentTtlSeconds, 60)));
    ...
    try {
      stringRedisTemplate
          ...
          .set(
              ...
              Duration.ofSeconds(safeSeconds(resultTtlSeconds, 60)));
    } catch (Exception ex) {
    ...
    try {
      ...
      stringRedisTemplate
          ...
          .putAll(
              key,
              java.util.Map.of(
                  STATUS_HASH_USER_ID, String.valueOf(userId), STATUS_HASH_STATUS, status));
