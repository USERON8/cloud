...
class PaymentOrderServiceImplTest {
    ...
    @Mock
    private PaymentSuccessTxProducer paymentSuccessTxProducer;
    ...
    @BeforeEach
    void setUp() {
        service = new PaymentOrderServiceImpl(
                paymentOrderMapper,
                paymentRefundMapper,
    ...
    void createPaymentOrderShouldReturnCachedResult() {
        ...
        when(orderStatusRemoteService.getSubOrderStatus(command.getMainOrderNo(), command.getSubOrderNo()))
                .thenReturn(status);
        when(paymentSecurityCacheService.allowRateLimit(command.getUserId())).thenReturn(true);
        when(paymentSecurityCacheService.getCachedResult(command.getMainOrderNo() + ":" + command.getSubOrderNo()))
                .thenReturn(88L);
