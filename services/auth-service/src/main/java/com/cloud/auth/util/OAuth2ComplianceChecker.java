...
public class OAuth2ComplianceChecker {
  ...
  private static final Set<String> REQUIRED_BASE_SCOPES =
      Set.of("SCOPE_openid", "SCOPE_profile", "SCOPE_read");
  ...
  private void validateRoleFormat(OAuth2ComplianceResult result, Set<String> authorities) {
    List<String> roles = authorities.stream().filter(auth -> auth.startsWith(ROLE_PREFIX)).toList();
    if (roles.isEmpty()) {
  ...
  private void validateScopeFormat(OAuth2ComplianceResult result, Set<String> authorities) {
    List<String> scopes =
        authorities.stream().filter(auth -> auth.startsWith(SCOPE_PREFIX)).toList();
    if (scopes.isEmpty()) {
  ...
  private void validateRoleSpecificPermissions(
      OAuth2ComplianceResult result, Set<String> authorities, List<String> roles) {
    ...
    } else if (!hasPermission(authorities, "user:profile")
        && !hasPermission(authorities, "user:address")) {
      result.addWarning("User missing user permissions");
    }
  }

  private void validatePermissionInheritance(
      OAuth2ComplianceResult result, Set<String> authorities, List<String> roles) {
    Set<String> normalizedRoles =
        roles.stream()
            .map(String::toUpperCase)
