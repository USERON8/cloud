...
public class JwtPasswordConfig {
  ...
  public KeyPair keyPair() {
    ...
    if (!allowGeneratedKeypair) {
      throw new IllegalStateException(
          "JWT key pair is not configured. Set app.jwt.keys.private-key-base64/public-key-base64");
    }

    log.warn(
        "JWT key pair is not configured, generating ephemeral key pair for this instance only");
  ...
  private JwtAuthenticationConverter buildJwtAuthenticationConverter(
    ...
    JwtAuthenticationConverter converter =
        JwtAuthorityUtils.buildJwtAuthenticationConverter(true, includeAuthoritiesClaim, null);
  ...
  private KeyPair loadKeyPairFromConfig() {
    try {
      ...
      RSAPublicKey publicKey =
          StrUtil.isNotBlank(configuredPublicKey)
  ...
  private RSAPublicKey derivePublicKey(RSAPrivateKey privateKey) throws Exception {
    if (!(privateKey instanceof RSAPrivateCrtKey crtKey)) {
      throw new IllegalStateException(
          "Public key is not configured and cannot be derived from private key type");
    }
