...
public class AuthorizationCodeFlowService {
    ...
    private AuthorizationSessionResponseDTO createAuthorizationSession(
        ...
        RegisteredClient registeredClient =
                validateAuthorizationRequest(clientId, redirectUri, state, scope, codeChallenge, codeChallengeMethod);
        ...
        UriComponentsBuilder builder = UriComponentsBuilder
                .fromUriString(resolveAuthorizationEndpoint(request))
    ...
    private RegisteredClient validateAuthorizationRequest(
        ...
        if (!registeredClient.getAuthorizationGrantTypes().contains(AuthorizationGrantType.AUTHORIZATION_CODE)) {
            throw new ValidationException("client_id", clientId, "client does not support authorization_code");
        }
        ...
        if (registeredClient.getClientSettings().isRequireProofKey()) {
            if (StrUtil.isBlank(codeChallenge)) {
                throw new ValidationException("code_challenge", codeChallenge, "code_challenge cannot be blank");
            }
            String method = resolveCodeChallengeMethod(codeChallengeMethod);
            if (!"S256".equalsIgnoreCase(method)) {
                throw new ValidationException("code_challenge_method", method, "code_challenge_method must be S256");
            }
        }
