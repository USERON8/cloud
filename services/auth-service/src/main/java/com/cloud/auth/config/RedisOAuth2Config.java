...
public class RedisOAuth2Config {
  ...
  public OAuth2AuthorizationService authorizationService() {
    return new RedisOAuth2AuthorizationService(
        oauth2MainRedisTemplate(), registeredClientRepository, authorizationServerSettings);
  }
    ...
    public String getConnectionInfo() {
      try {
        return String.format(
            "Redis connection factory: %s", connectionFactory.getClass().getSimpleName());
      } catch (Exception e) {
  ...
  public static class RedisHealthChecker {
    ...
    public RedisStats getStats() {
      return new RedisStats(
          isHealthy(),
          isHealthy() ? "healthy" : "Redis health check failed",
    ...
    public static class RedisStats {
      ...
      public String toString() {
        return String.format(
            "RedisStats{healthy=%s, status='%s', checkTime=%d}", healthy, status, checkTime);
      }
    }
  }
}
