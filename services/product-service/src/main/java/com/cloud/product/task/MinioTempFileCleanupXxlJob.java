    ...
    public void cleanTempFiles() {
        ...
        try {
            ...
            for (Result<Item> result : results) {
                ...
                scanned++;
                Instant lastModified = item.lastModified() == null
                        ? null
            ...
            for (Result<DeleteError> errorResult : errors) {
                ...
                failed++;
                log.warn("Remove temp object failed: object={}, error={}",
                        error.objectName(), error.message());
            }

            int deleted = toDelete.size() - failed;
            XxlJobHelper.log("minioTempFileCleanJob finished, scanned=%d, deleted=%d, failed=%d",
