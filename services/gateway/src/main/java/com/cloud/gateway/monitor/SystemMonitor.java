...
public class SystemMonitor {
    ...
    try {
      ...
      double cpuUsage =
          osBean.getSystemLoadAverage() > 0
              ? Math.min(
                  osBean.getSystemLoadAverage() * 100 / osBean.getAvailableProcessors(), 100.0)
              : 0.0;
      ...
      log.debug(
          "Gateway system resources - cpu: {:.1f}%, memory: {:.1f}%, uptime: {} min",
  ...
  private void checkResourceAlerts(double cpuUsage, double memoryUsage) {
    ...
    if (cpuUsage > RESOURCE_CRITICAL_THRESHOLD || memoryUsage > RESOURCE_CRITICAL_THRESHOLD) {
      log.error(
          "Gateway resources are critical. CPU: {:.1f}%, memory: {:.1f}%", cpuUsage, memoryUsage);
    }
  }
  ...
  public String getSystemStatusSummary() {
    ...
    try {
      ...
      summary.append(
          String.format("  System resources: CPU %.1f%%, memory %.1f%%%n", cpuUsage, memoryUsage));
    ...
    try {
      ...
      double memoryUsage =
          (double) memoryBean.getHeapMemoryUsage().getUsed()
              / Math.max(memoryBean.getHeapMemoryUsage().getMax(), 1)
              * 100;
      ...
      ThreadPoolTaskExecutor[] executors = {
        gatewayMonitorExecutor, gatewayLogExecutor, gatewayStatisticsExecutor
      };
      ...
      for (int i = 0; i < pools.length; i++) {
        ...
        double activeRate =
            pool.getMaximumPoolSize() > 0
                ...
                : 0.0;
        double queueRate =
            (double) pool.getQueue().size() / Math.max(executor.getQueueCapacity(), 1) * 100;

        if (activeRate > THREAD_POOL_ACTIVE_WARNING_THRESHOLD
            || queueRate > THREAD_POOL_QUEUE_WARNING_THRESHOLD) {
          return true;
        }
      }
    } catch (Exception e) {
