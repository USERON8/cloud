...
public class OrderCompletedConsumer {
    ...
    /**
     * 订单完成消费者函数
     * 对应绑定名称: orderCompletedConsumer-in-0
     */
    @Bean
    public Consumer<Message<OrderCompletedEvent>> orderCompletedConsumer() {
        return message -> {
            try {
                ...
                if (processed) {
                    log.info("✅ 库存解冻扣减成功 - 订单ID: {}, 订单号: {}, 商品数量: {}, 追踪ID: {}", 
                            orderId, orderNo, event.getStockDeductions().size(), traceId);
                } else {
