 ...
 */
@Configuration
public class RocketMQLogConfig {
    ...
    /**
     * 库存类型变更日志消息生产者
     * 用于异步发送库存类型变更事件到RocketMQ，供日志服务消费并记录日志
     * 
     * @return Supplier函数式接口，提供消息构建方式
     */
    @Bean
    public Supplier<Message<StockTypeChangeEvent>> stockTypeChangeProducer() {
        return () -> {
            // 构建一个空的StockTypeChangeEvent消息，实际使用时通过StreamBridge发送具体消息
            return MessageBuilder.withPayload(new StockTypeChangeEvent())
                    .setHeader("traceId", UUID.randomUUID().toString())
                    .build();
        };
    }
