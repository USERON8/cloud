...
CREATE TABLE `sku_specification` (
    ...
    INDEX `idx_category_id` (`category_id`),
    INDEX `idx_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='SKU规格定义表';
...
CREATE TABLE `attribute_template` (
    ...
    INDEX `idx_category_id` (`category_id`),
    INDEX `idx_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='属性模板表';
...
-- 插入品牌数据
INSERT INTO `brand` (`brand_name`, `brand_name_en`, `country`, `status`, `is_hot`, `is_recommended`) VALUES
('华为', 'HUAWEI', '中国', 1, 1, 1),
('小米', 'Xiaomi', '中国', 1, 1, 1),
('苹果', 'Apple', '美国', 1, 1, 1),
('OPPO', 'OPPO', '中国', 1, 1, 0),
...
-- 插入店铺数据
INSERT INTO `merchant_shop` (`merchant_id`, `shop_name`, `contact_phone`, `address`, `status`) VALUES
(1, '华为官方旗舰店', '400-123-4567', '深圳市南山区', 1),
(2, '小米官方旗舰店', '400-234-5678', '北京市海淀区', 1),
(3, '苹果官方旗舰店', '400-345-6789', '上海市浦东新区', 1);

-- 插入商品数据
INSERT INTO `products` (`id`, `shop_id`, `product_name`, `product_description`, `price`, `original_price`, `stock_quantity`, `category_id`, `brand_id`, `status`, `sales_count`) VALUES
(1, 1, '华为Mate 60 Pro', '华为旗舰手机，强大影像系统', 6999.00, 7999.00, 100, 2, 1, 1, 1230),
