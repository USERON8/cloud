package com.cloud.search.aspect;

import com.cloud.search.annotation.MultiLevelCacheEvict;
import com.cloud.search.annotation.MultiLevelCachePut;
...
public class MultiLevelCacheAspect {
    ...
    public Object handleCacheable(ProceedingJoinPoint joinPoint, MultiLevelCacheable multiLevelCacheable) throws Throwable {
        ...
        if (multiLevelCacheable.enableLocalCache()) {
            ...
            if (result != null) {
                log.debug("从L1缓存获取搜索数据, cacheName={}, key={}", cacheName, cacheKey);
                return result;
            }
        }
        ...
        if (result != null) {
            log.debug("从L2缓存获取搜索数据, cacheName={}, key={}", cacheName, cacheKey);
            // 回种到L1缓存
            if (multiLevelCacheable.enableLocalCache()) {
        ...
        // 3. 执行原方法（从ES搜索）
        log.debug("从ES搜索数据, cacheName={}, key={}", cacheName, cacheKey);
    ...
    private void evictCache(String cacheName, String key, boolean allEntries) {
        if (allEntries) {
            ...
            log.debug("清空所有搜索缓存, cacheName={}", cacheName);
        } else {
            // 删除指定key
            Cache cache = localCacheManager.getCache(cacheName);
            if (cache != null) {
            ...
            log.debug("删除搜索缓存, cacheName={}, key={}", cacheName, key);
        }
    }
    ...
    private String buildRedisKey(String cacheName, String key) {
        return "search-cache:" + cacheName + ":" + key;
    }
