...
public class LockException extends BusinessException {
  ...
  public static LockException releaseFailed(String lockKey) {
    return new LockException(LOCK_RELEASE_FAILED, String.format("閲婃斁鍒嗗竷寮忛攣澶辫触锛岄攣閿? %s", lockKey));
  }

  public static LockException renewFailed(String lockKey) {
    return new LockException(LOCK_RENEW_FAILED, String.format("缁湡鍒嗗竷寮忛攣澶辫触锛岄攣閿? %s", lockKey));
  }

  public static LockException operationError(String lockKey, String operation, Throwable cause) {
    return new LockException(
        LOCK_OPERATION_ERROR,
        String.format("鍒嗗竷寮忛攣鎿嶄綔寮傚父锛岄攣閿? %s, 鎿嶄綔: %s", lockKey, operation),
        cause);
  }
}
