...
public class BaseAsyncConfig {
    ...
    if (asyncProperties != null) {
      ...
      executor =
          createThreadPoolTaskExecutor(
              Math.max(4, processors), processors * 3, 300, "async-default-");
    }
  ...
  protected ThreadPoolTaskExecutor createThreadPoolTaskExecutorFromConfig(
    ...
    executor.setThreadNamePrefix(
        prefix != null && !prefix.isEmpty() ? prefix : defaultThreadNamePrefix);

    executor.setKeepAliveSeconds(config.getKeepAliveSeconds());
    executor.setAllowCoreThreadTimeOut(config.isAllowCoreThreadTimeOut());

    executor.setRejectedExecutionHandler(
        getRejectedExecutionHandler(config.getRejectedExecutionHandler()));
  ...
  protected ThreadPoolTaskExecutor createCommonAsyncExecutor() {
    ...
    return createThreadPoolTaskExecutor(
        Math.max(2, processors / 2), processors * 2, 200, "common-async-");
  }

  protected ThreadPoolTaskExecutor createQueryExecutor(String threadNamePrefix) {
    int processors = Runtime.getRuntime().availableProcessors();
    return createThreadPoolTaskExecutor(
        Math.max(4, processors), processors * 4, 500, threadNamePrefix);
  }

  protected ThreadPoolTaskExecutor createWriteExecutor(String threadNamePrefix) {
    return createThreadPoolTaskExecutor(4, 16, 500, threadNamePrefix);
  }

  protected ThreadPoolTaskExecutor createIOExecutor(String threadNamePrefix) {
    int processors = Runtime.getRuntime().availableProcessors();
    return createThreadPoolTaskExecutor(processors * 2, processors * 4, 300, threadNamePrefix);
  }

  protected ThreadPoolTaskExecutor createCPUExecutor(String threadNamePrefix) {
    int processors = Runtime.getRuntime().availableProcessors();
    return createThreadPoolTaskExecutor(processors, processors + 1, 100, threadNamePrefix);
  }
  ...
  protected ThreadPoolTaskExecutor createConfiguredExecutor(
      String executorKey,
      int corePoolSize,
      int maxPoolSize,
      int queueCapacity,
    ...
    ThreadPoolTaskExecutor executor =
        createThreadPoolTaskExecutor(corePoolSize, maxPoolSize, queueCapacity, threadNamePrefix);
  ...
  private void applyExecutorOverrides(ThreadPoolTaskExecutor executor, String executorKey) {
    if (asyncProperties == null
        || asyncProperties.getExecutors() == null
        || executorKey == null
        || executorKey.isBlank()) {
      return;
    }
    ...
    executor.setRejectedExecutionHandler(
        getRejectedExecutionHandler(override.getRejectedExecutionHandler()));
