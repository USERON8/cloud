...
public class DubboConsumerExceptionFilter implements Filter {
    ...
    try {
      ...
      if (e.isTimeout()) {
        log.error(
            ...
            e);
        throw RemoteException.timeout(ResultCode.REMOTE_SERVICE_TIMEOUT, target, elapsed, e);
      }
      if (e.isLimitExceed()) {
      ...
      if (e.isNetwork() || e.isNoInvokerAvailableAfterFilter()) {
        ...
        throw RemoteException.unavailable(ResultCode.REMOTE_SERVICE_UNAVAILABLE, target, e);
      }
  ...
  private void warnSlowIfNeeded(
      String service, String method, long elapsed, Invocation invocation) {
    long threshold = resolveSlowThresholdMs(invocation);
    if (elapsed > threshold) {
      log.warn(
          "[DUBBO][SLOW] {}.{} elapsed={}ms threshold={}ms", service, method, elapsed, threshold);
    }
  }
