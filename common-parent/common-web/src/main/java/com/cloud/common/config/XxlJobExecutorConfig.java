...
public class XxlJobExecutorConfig {
    ...
    public XxlJobSpringExecutor xxlJobSpringExecutor(XxlJobProperties properties, Environment environment) {
        ...
        String appName = firstNonBlank(executorProperties.getAppname(), environment.getProperty("spring.application.name"));
        if (appName == null) {
            throw new IllegalStateException("xxl.job.executor.appname or spring.application.name must be configured");
        }
        ...
        log.info("XXL-JOB executor enabled: appName={}, adminAddresses={}, port={}",
                appName,
