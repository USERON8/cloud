...
public interface OutboxEventMapper extends BaseMapper<OutboxEvent> {
    ...
    @Select("SELECT id, event_id, aggregate_type, aggregate_id, event_type, payload, status, retry_count, next_retry_at, "
    ...
    @Update("UPDATE outbox_event SET status = 'SENT', updated_at = NOW() "
            + "WHERE id = #{id} AND deleted = 0")
    int markSent(@Param("id") Long id);

    @Update("UPDATE outbox_event SET status = #{status}, retry_count = #{retryCount}, next_retry_at = #{nextRetryAt}, "
