...
public class JdbcDeadLetterService implements DeadLetterService {
  ...
  @Override
  public void record(
      String topic, String msgId, String payload, DeadLetterReason reason, Throwable error) {
    ...
    try {
      jdbcTemplate.update(
          INSERT_SQL, safeTopic, safeMsgId, safePayload, reasonValue, errorMsg, serviceName);
    } catch (Exception ex) {
      log.warn(
