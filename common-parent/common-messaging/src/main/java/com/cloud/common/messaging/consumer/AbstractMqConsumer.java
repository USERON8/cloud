...
public abstract class AbstractMqConsumer<T> implements RocketMQListener<MessageExt> {
    ...
    @Autowired
    protected MessageIdempotencyService messageIdempotencyService;
    ...
    public final void onMessage(MessageExt msgExt) {
        ...
        if (!messageIdempotencyService.tryAcquire(namespace, idempotentKey)) {
            log.info(
                    "[MQ][{}] Duplicate message, skip msgId={} key={}",
                    topic,
                    msgId,
                    idempotentKey);
        ...
        try {
            ...
            log.error(
                    "[MQ][{}][SYS-ACK] non-retryable system exception, msgId={}",
                    topic,
                    msgId,
                    ex);
            deadLetterService.record(
                    topic, msgId, payload, DeadLetterReason.SYS_NONRETRYABLE, ex);
            increment(
                    "mq.consume.sys_fail",
                    "topic",
                    topic,
