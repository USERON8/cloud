...
public abstract class AbstractMqConsumer<T> implements RocketMQListener<MessageExt> {
  ...
  public final void onMessage(MessageExt msgExt) {
    ...
    MDC.put(TraceIdUtil.TRACE_ID_KEY, resolveTraceId(msgExt, msgId));
    try {
      T payload;
      try {
  ...
  protected String resolveTraceId(MessageExt msgExt, String msgId) {
    if (msgExt != null) {
      String traceId = msgExt.getProperty("traceId");
      if (traceId == null || traceId.isBlank()) {
        traceId = msgExt.getProperty(TraceIdUtil.TRACE_HEADER);
      }
      if (traceId != null && !traceId.isBlank()) {
        return traceId.trim();
      }
