...
本项目采用**简化支付模式**，无需对接支付宝/微信等第三方支付网关，通过前端直接调用支付成功/失败/退款接口来实现完整的业务闭环。
...
2. 预扣库存 → 库存服务预扣库存
3. 创建支付单 → 支付服务创建支付记录
4. 前端模拟支付 → 用户在前端页面点击确认支付
5. 调用成功接口 → 前端调用 POST /payments/{id}/success
6. 更新支付状态 → 支付服务更新为已支付
7. 通知订单服务 → 通过MQ通知订单服务
8. 更新订单状态 → 订单状态更新为已支付
```
...
@DistributedLock(key = "'payment:success:' + #id")
public Result<Boolean> paymentSuccess(@PathVariable Long id) {
    return paymentService.processPaymentSuccess(id);
}
```
...
@DistributedLock(key = "'payment:fail:' + #id")
public Result<Boolean> paymentFail(@PathVariable Long id,
                                     @RequestParam(required = false) String failReason) {
    return paymentService.processPaymentFailed(id, failReason);
}
```
...
@DistributedLock(key = "'payment:refund:' + #id")
public Result<Boolean> refundPayment(@PathVariable Long id,
                                      @RequestParam BigDecimal refundAmount,
                                      @RequestParam(required = false) String refundReason) {
    return paymentService.processRefund(id, refundAmount, refundReason);
}
```

### 安全考虑

1. **分布式锁**: 所有支付操作必须使用分布式锁，防止并发问题
2. **幂等性**: 支付成功/失败/退款接口必须保证幂等性
3. **状态校验**: 处理前必须校验当前支付状态
4. **权限控制**: 需要验证用户权限，防止越权操作
5. **日志记录**: 所有支付操作必须记录完整日志

### 前端交互指导

1. **创建订单后**：显示支付页面，包含订单信息和支付金额
2. **模拟支付**：用户点击“确认支付”按钮
3. **调用接口**：前端调用 `POST /payments/{id}/success`
4. **处理结果**：根据响应显示支付成功或失败
5. **跳转页面**：支付成功后跳转到订单详情页

### 注意事项

- ⚠️ **简化模式限制**: 本模式仅适用于演示、测试和快速部署场景
- 🔒 **生产环境**: 生产环境建议集成真实支付网关
- 🔧 **扩展性**: 代码设计保留了扩展性，可轻松集成支付宝/微信
- 📊 **监控告警**: 建议配置支付失败率等关键指标监控
