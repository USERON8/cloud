...
public class LogCollectionProducer {
     ...
     * 发送商品业务日志
     */
    public void sendProductLog(LogCollectionEvent event) {
    ...
    private void sendLogEvent(LogCollectionEvent event, String tag) {
        try {
            ...
            if (sent) {
                log.debug("✅ 商品日志发送成功 - 日志类型: {}, 操作: {}, 追踪ID: {}",
                        event.getLogType(), event.getOperation(), event.getTraceId());
            } else {
                log.error("❌ 商品日志发送失败 - 日志类型: {}, 操作: {}, 追踪ID: {}",
                        event.getLogType(), event.getOperation(), event.getTraceId());
            ...
            throw new MessageSendException("发送商品日志异常", e);
        }
    }
