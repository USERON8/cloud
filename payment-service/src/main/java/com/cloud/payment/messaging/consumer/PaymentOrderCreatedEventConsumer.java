...
public class PaymentOrderCreatedEventConsumer {

    private final PaymentService paymentService;

    /**
     * 支付订单创建消费者函数
     * 对应绑定名称: paymentOrderCreatedConsumer-in-0
     */
    @Bean("paymentOrderCreatedMessageConsumer")
    public Consumer<Message<OrderCreatedEvent>> paymentOrderCreatedMessageConsumer() {
        return message -> {
            try {
