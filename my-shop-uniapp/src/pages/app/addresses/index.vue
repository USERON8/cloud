...
async function loadAddresses(): Promise<void> {
  ...
  try {
    ...
    toast(error instanceof Error ? error.message : '加载地址失败')
  } finally {
    loading.value = false
  }
}
  ...
  <AppShell title="Addresses">
    <view class="panel glass-card">
      <view class="header">
        ...
        <button class="btn-outline" @click="loadAddresses">刷新</button>
      </view>

      <view v-if="rows.length === 0" class="empty">
        <text class="text-muted">暂无地址</text>
      </view>
      ...
      <view v-else class="list">
        <view v-for="item in rows" :key="item.id" class="row">
          ...
          <text class="meta" v-if="item.isDefault === 1">默认地址</text>
        </view>
      </view>
    </view>
  </AppShell>
